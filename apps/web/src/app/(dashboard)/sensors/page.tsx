...
import { Activity, Droplets, Thermometer, Beaker, Leaf, WifiOff, AlertTriangle } from "lucide-react";
  ...
  return (
    <motion.div variants={container} initial="hidden" animate="show" className="space-y-6 pb-20 lg:pb-0">
      <motion.div variants={item} className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <h1 className="font-sans text-2xl font-bold tracking-tight [text-wrap:balance]">Sensor</h1>
          <p className="mt-1 text-sm leading-6 text-muted-fg [text-wrap:pretty]">Konfigurasi threshold, kalibrasi, dan status MQTT.</p>
        </div>
        <div className="flex items-center gap-2">
          <Badge variant="success" className="gap-1.5">
            <span className="h-2 w-2 rounded-full bg-success animate-pulse" /> MQTT ● Terhubung
          </Badge>
          <span className="hidden text-xs text-muted-fg sm:inline">broker: mqtt://101.50.2.190:1883</span>
        </div>
      </motion.div>
      ...
      <motion.div variants={item}>
        <Card muted className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm font-semibold [text-wrap:balance]">Status MQTT</p>
              <p className="text-xs leading-4 text-muted-fg [text-wrap:pretty]">Terhubung · 6 sensor terdaftar · interval 60 dtk</p>
            </div>
          </div>
        ...
        <div className="flex flex-wrap gap-2">
          {["Semua", "pH", "NPK", "PPM", "Tandon", "Suhu", "Kelembapan"].map((t) => (
            <button
              ...
              onClick={() => setFilter(t)}
              className={[
                "h-11 rounded-pill border px-3.5 text-xs font-semibold transition-colors",
                filter === t ? "bg-primary text-primary-fg border-primary shadow-sm" : "bg-background hover:bg-muted",
              ].join(" ")}
            >
              {t}
            </button>
      ...
      {loading ? (
        ...
        <motion.div variants={item}>
          <EmptyState
            variant="sensor"
            title="Tidak ada sensor"
            description="Tidak ditemukan sensor dengan filter ini. Coba ubah kata kunci atau tipe."
        ...
        >
          {filtered.map((s) => {
            const Icon = tipeIcon[s.tipe] ?? Activity;
            const badgeVariant = s.status === "online" ? "success" : s.status === "warning" ? "warning" : "destructive";
            const label = s.status === "online" ? "Online" : s.status === "warning" ? "Perhatian" : "Offline";
            return (
              <motion.div key={s.id} variants={item} whileHover={{ scale: 1.02 }} transition={{ type: "spring", stiffness: 300, damping: 20 }}>
                <Card className="relative h-full">
                  <CardHeader>
                    <CardTitle className="flex items-center gap-2 [text-wrap:balance]">
                      <span className="flex h-7 w-7 items-center justify-center rounded-lg bg-primary-soft text-primary-soft-fg">
                        <Icon className="h-4 w-4" />
                      </span>
                      {s.name}
                    </CardTitle>
                    <Badge variant={badgeVariant as "success" | "warning" | "destructive"}>{label}</Badge>
                  </CardHeader>
                  ...
                  <p className="mt-1 text-xs leading-4 text-muted-fg [text-wrap:pretty]">
                    {s.lahan} · {s.lastSeen}
                  </p>
      ...
      <motion.div
        ...
        className="rounded-card border bg-background p-5 shadow-sm"
      >
        <h3 className="flex items-center gap-2 font-sans text-base font-semibold [text-wrap:balance]">
          <Beaker className="h-4 w-4 text-primary" /> Atur Threshold (Mock)
        </h3>
        <p className="mt-1 text-sm leading-6 text-muted-fg [text-wrap:pretty]">Simpan ambang batas untuk alert. Belum terhubung ke backend — mock UI.</p>
        <form
          onSubmit={(e) => {
            e.preventDefault();
            setSaved(true);
        ...
        {saved && <p className="mt-3 flex items-center gap-1.5 text-sm font-medium text-success">✓ Threshold tersimpan (mock)</p>}
      </motion.div>
    </motion.div>
  );
}
